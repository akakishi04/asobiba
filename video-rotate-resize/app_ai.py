...
from ai_backends import (
    AI_CHOICES, AI_NONE, AI_RVRT, AI_RVRT_SEEDVR2, AI_SEEDVR2,
    RVRT_TASKS, AIConfig, build_rvrt_command, build_seedvr2_command,
    normalize_png_sequence, resolve_python, rvrt_result_dir, validate_for_mode,
)
from video_tool import (
    VideoToolError, build_extract_png_command, build_ffmpeg_command,
    build_frames_to_lossless_video_command, build_frames_to_video_command,
    build_lossless_transform_command, find_executable, nvenc_available,
    probe_media, transformed_dimensions, validate_target_resolution,
)

ROTATIONS = {"なし": "none", "時計回り 90°": "cw90", "反時計回り 90°": "ccw90", "180°": "180"}
RESOLUTIONS = {
    "元の解像度": None, "1080p (1920×1080)": (1920, 1080),
    "720p (1280×720)": (1280, 720), "4K (3840×2160)": (3840, 2160),
    "カスタム": "custom",
}
...
class App:
    ...
    def _ui(self):
        ...
        self._combo_row(s, 3, "エンコーダー", self.encoder,
        ...
        ttk.Combobox(ar, textvariable=self.ai_mode, values=list(AI_CHOICES),
                     state="readonly").grid(row=0, column=0, sticky="ew")
        ttk.Button(ar, text="AI設定...", command=self._settings).grid(row=0, column=1, padx=(8, 0))
        ttk.Label(s, textvariable=self.ai_state, foreground="#555").grid(
            row=5, column=1, sticky="w", padx=(10, 0))
        ttk.Label(
            s,
            text="AIなしは従来のFFmpeg変換。AI時は最終解像度を維持し、元動画の音声を戻します。",
            foreground="#555",
        ...
        self.stop = ttk.Button(b, text="キャンセル", command=self._cancel, state="disabled")
    ...
    def _path_row(self, parent, row, label, var, command):
        ...
        ttk.Button(parent, text="参照...", command=command).grid(row=row, column=2, pady=3)
    ...
    def _custom_state(self):
        ...
        self.cw_entry.configure(state=state); self.ch_entry.configure(state=state)

    def _pick_input(self):
        p = filedialog.askopenfilename(
            title="入力動画", filetypes=[("動画", "*.mp4 *.mov *.mkv *.m4v *.avi *.webm"), ("すべて", "*.*")])
        if not p:
            return
        self.input.set(p)
        src = Path(p)
    ...
    def _pick_output(self):
        ...
        p = filedialog.asksaveasfilename(
            title="出力先", initialfile=Path(current).name,
    ...
    def _start(self):
        ...
        try:
            ...
            ffmpeg = find_executable("ffmpeg"); find_executable("ffprobe")
            ...
            if mode == AI_NONE:
                if rot == "none" and target is None:
                    raise VideoToolError("回転、解像度変更、またはAI鮮明化を指定してください。")
            ...
            self._launch(self._ai_worker, src, dst, rot, target, mode, enc, fps, dims, ffmpeg)
        except VideoToolError as e:
            messagebox.showerror("設定エラー", str(e))

    def _launch(self, worker, *args):
        self.cancelled = False; self.running = True
    ...
    def _ai_worker(self, src, dst, rot, target, mode, enc, fps, dims, ffmpeg):
        ...
        try:
            with tempfile.TemporaryDirectory(prefix=".video_ai_", dir=dst.parent) as td:
                t = Path(td); frames = None
                if mode in {AI_RVRT, AI_RVRT_SEEDVR2}:
                    self._stage(5, "RVRT用フレーム展開...")
                    name = "clip_" + uuid.uuid4().hex[:10]
                    root = t / "rvrt"; inp = root / name; inp.mkdir(parents=True)
                    self._run(build_extract_png_command(src, inp / "frame%06d.png", rot, target, ffmpeg), "Frame extract")
                    self._stage(20, "RVRT復元中...")
                    cleanup = rvrt_result_dir(self.config, name)
                    shutil.rmtree(cleanup, ignore_errors=True)
                    cmd, cwd = build_rvrt_command(self.config, root)
                    self._run(cmd, "RVRT", cwd)
                    frames = t / "rvrt_out"
                    normalize_png_sequence(cleanup, frames)

                if mode in {AI_SEEDVR2, AI_RVRT_SEEDVR2}:
                    if mode == AI_RVRT_SEEDVR2:
                        self._stage(55, "SeedVR2用中間映像作成...")
                        seed_in = t / "rvrt_seed.mkv"
                        self._run(build_frames_to_lossless_video_command(
                            frames / "frame%06d.png", fps, seed_in, ffmpeg), "Lossless intermediate")
                    else:
                        seed_in = src
                        if rot != "none" or target is not None:
                            self._stage(8, "SeedVR2用中間映像作成...")
                            seed_in = t / "seed_input.mkv"
                            self._run(build_lossless_transform_command(
                                src, seed_in, rot, target, ffmpeg), "Lossless transform")
                    self._stage(62 if mode == AI_RVRT_SEEDVR2 else 20, "SeedVR2復元中...")
                    out = t / "seed_out"; out.mkdir()
                    cmd, cwd = build_seedvr2_command(self.config, seed_in, out, min(dims))
                ...
                self._run(build_frames_to_video_command(
                    frames / "frame%06d.png", fps, src, dst, enc, dims, ffmpeg), "Final encode")
                self._done(dst)
        except Cancelled:
            self._cancelled()
        except Exception as e:
    ...
    def _done(self, dst):
        ...
        self._log_async(f"\n完了: {dst}\n")
        self.root.after(0, lambda: messagebox.showinfo("完了", f"保存しました:\n{dst}"))
    ...
    def _settings(self):
        w = tk.Toplevel(self.root); w.title("AI設定"); w.geometry("760x500"); w.grab_set()
        f = ttk.Frame(w, padding=14); f.pack(fill="both", expand=True); f.columnconfigure(1, weight=1)
        vals = {
            "rr": tk.StringVar(value=self.config.rvrt_repo),
        ...
        rt = tk.StringVar(value=rev.get(self.config.rvrt_task, next(iter(RVRT_TASKS))))

        def row(r, label, key, directory=False):
            ttk.Label(f, text=label).grid(row=r, column=0, sticky="w", pady=3)
            ttk.Entry(f, textvariable=vals[key]).grid(row=r, column=1, sticky="ew", padx=8, pady=3)
            def pick():
        ...
        ttk.Label(f, text="RVRT", font=("", 10, "bold")).grid(row=0, column=0, sticky="w")
        row(1, "RVRTフォルダ", "rr", True); row(2, "RVRT Python", "rp")
        ttk.Label(f, text="RVRTモデル").grid(row=3, column=0, sticky="w")
        ttk.Combobox(f, textvariable=rt, values=list(RVRT_TASKS), state="readonly").grid(
            row=3, column=1, sticky="ew", padx=8)
        ttk.Separator(f).grid(row=4, column=0, columnspan=3, sticky="ew", pady=10)
        ttk.Label(f, text="SeedVR2", font=("", 10, "bold")).grid(row=5, column=0, sticky="w")
        row(6, "SeedVR2フォルダ", "sr", True); row(7, "SeedVR2 Python", "sp")
        ttk.Label(f, text="Model").grid(row=8, column=0, sticky="w")
        ttk.Entry(f, textvariable=vals["sm"]).grid(row=8, column=1, sticky="ew", padx=8)
        adv = ttk.Frame(f); adv.grid(row=9, column=1, sticky="w", padx=8, pady=6)
        for label, key in [("Batch", "sb"), ("BlockSwap", "ss"), ("処理短辺(0=ネイティブ)", "sx")]:
            ttk.Label(adv, text=label).pack(side="left")
            ttk.Entry(adv, width=7, textvariable=vals[key]).pack(side="left", padx=(3, 10))
        ttk.Label(
            f, text="4070 Ti SUPER 16GB向け初期値: 3B FP8 / Batch 5 / BlockSwap 20 / VAE tiling。",
            foreground="#555").grid(row=10, column=0, columnspan=3, sticky="w", pady=8)

        def save():
            try:
                batch, swap, res = int(vals["sb"].get()), int(vals["ss"].get()), int(vals["sx"].get())
                if batch < 1 or swap < 0 or res < 0: raise ValueError
            except ValueError:
                messagebox.showerror("AI設定", "Batch>=1、BlockSwap/処理短辺>=0の整数にしてください。", parent=w)
                return
            self.config = AIConfig(
                rvrt_repo=vals["rr"].get().strip(), rvrt_python=vals["rp"].get().strip(),
                rvrt_task=RVRT_TASKS[rt.get()], seedvr2_repo=vals["sr"].get().strip(),
                seedvr2_python=vals["sp"].get().strip(), seedvr2_model=vals["sm"].get().strip(),
                seedvr2_batch_size=batch, seedvr2_blocks_to_swap=swap,
                seedvr2_temporal_overlap=self.config.seedvr2_temporal_overlap,
                seedvr2_resolution_override=res, seedvr2_vae_tiling=True,
                seedvr2_vae_tile_size=self.config.seedvr2_vae_tile_size,
                seedvr2_vae_tile_overlap=self.config.seedvr2_vae_tile_overlap)
            self.config.save(self.config_path); self._refresh_ai_state(); w.destroy()

        bar = ttk.Frame(f); bar.grid(row=11, column=0, columnspan=3, sticky="e")
    ...
    def _refresh_ai_state(self):
        def ready(repo, marker, py):
            if not repo or not (Path(repo) / marker).is_file():
                return False
            try:
                resolve_python(py); return True
            except VideoToolError:
                return False
        r = ready(self.config.rvrt_repo, "main_test_rvrt.py", self.config.rvrt_python)
        s = ready(self.config.seedvr2_repo, "inference_cli.py", self.config.seedvr2_python)
        self.ai_state.set(f"RVRT: {'準備済み' if r else '未設定'} / SeedVR2: {'準備済み' if s else '未設定'}")

    def _close(self):
        if self.running and not messagebox.askyesno("終了", "変換を停止して終了しますか？"):
            return
        self.cancelled = True
        if self.proc: self._kill(self.proc)
        self.root.destroy()
